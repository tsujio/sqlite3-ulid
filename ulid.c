...
static unsigned char DECODE[130] = {
    0xff,
    0xff,
    0xff,
    0xff,
...
static void ulid_new(sqlite3_context *context, int argc, sqlite3_value **argv) {
    if (argc != 2) {
        sqlite3_result_error(context, "Required exactly 2 arguments", -1);
        return;
    }
    ...
    unsigned long long timestamp = sqlite3_value_int64(argv[0]);
    const unsigned char *randomness = sqlite3_value_blob(argv[1]);
...
int sqlite3_extension_init(
    ...
    res = sqlite3_create_function(db, "ulid_new", 2, SQLITE_UTF8 | SQLITE_DETERMINISTIC, NULL, ulid_new, NULL, NULL);
    if (res != SQLITE_OK) {
